...
class IrrigationManager:
    ...
    async def _async_sensor_changed(self, event):
        ...
        for zone_id, data in self.running_zones.items():
            # Manual AND scheduled zones both respect the soil moisture target
    ...
    async def _stop_zones(self, zones_to_stop):
        ...
        for zone_id, reason in zones_to_stop:
            ...
            is_manual = self.running_zones[zone_id].get("is_manual", False)
    ...
    async def async_manual_toggle(self, zone_id, state):
        ...
        if state:
            ...
            if zone_id in self.running_zones:
                del self.running_zones[zone_id]
            # Correctly record that the zone was watered so min_rest_days etc. work correctly
            zone["last_watered_at"] = dt_util.now().isoformat()
            zone["last_skipped_at"] = None
            zone["last_skipped_reason"] = None
            await self.store.save_irrigation(self.config)
    ...
    async def _async_check_irrigation(self, now):
        ...
        for zone_id, data in self.running_zones.items():
            if now - data["start_time"] >= data["duration"]:
                ...
                # Both manual AND scheduled zones stop when target moisture is reached
        ...
        for zone in self.config.get("zones", []):
            ...
            if override_state != "force_water":
                ...
                # ── Determine rain catch-up ───────────────────────────────────────
                rain_catchup = False
                last_skipped_str = zone.get("last_skipped_at")
                last_skipped_reason = zone.get("last_skipped_reason")
                if last_skipped_str and last_skipped_reason == "Regen":
                    try:
                        ls = dt_util.parse_datetime(last_skipped_str)
                        if ls:
                            days_since_skip = (now.date() - dt_util.as_local(ls).date()).days
                            if days_since_skip == 1:
                                rain_catchup = True
                    except Exception:
                        pass
                ...
                if has_sensor:
                    ...
                    if now.weekday() < len(weekday_schedule) and not weekday_schedule[now.weekday()] and not heat_override and not rain_catchup:
                    ...
                    if not is_scheduled_today and not heat_override and not rain_catchup:
                        continue
