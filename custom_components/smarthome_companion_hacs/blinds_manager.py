...
class BlindsManager:
    ...
    def _get_effective_config(self, entity_id, base_config):
        simulation_mgr = self.hass.data.get(DOMAIN, {}).get("simulation_manager")
        if not simulation_mgr or not simulation_mgr.is_active:
            return base_config

        sim_config = simulation_mgr.get_blind_config(entity_id)
        if not sim_config:
            return base_config
    ...
    def calculate_times(self, entity_id, config, date_val=None):
        config = self._get_effective_config(entity_id, config)
        import random
        now = dt_util.now()
        if date_val is None:
            date_val = now.date()
                        ...
                        if dt_str:
                            try:
                                ...
                                if f_dt:
                                    ...
                                    f_date = f_dt.date()
                                    # Always set cloud_cov per entry to avoid using stale value from previous iteration
    ...
    async def _regular_loop(self, now):
        ...
        # Nutze Datum+Stunde als Key, damit es jeden Tag erneut ausgeführt wird
        holiday_update_key = f"{now.date().isoformat()}-{now.hour}"
        if now.hour in (0, 12) and self._last_holiday_update_hour != holiday_update_key:
            self._last_holiday_update_hour = holiday_update_key
            await self._update_holidays()
        await self._update_weather_forecast()
    ...
    async def _evaluate_blind(self, entity_id, config, is_watchdog_check=False, is_state_change=False, force_correction=False):
                            ...
                            if s_time_str:
                                ...
                                if s_time and now >= s_time and plan.get("target_position") == 0:
                                    if is_opening and mem.get("ventilation_stopped_today") != now.date().isoformat():
                                        ...
                                        if current_position < ventilation_position:
                                            ...
                                            # Bereits höher - nichts tun, direkt als fertig markieren
        ...
        if is_morning_ventilation_time and mem.get("ventilation_initiated_today") != now.date().isoformat():
            ...
            # Nur hochfahren zur Lüftungsposition, niemals runterfahren!
            if is_opening_detected and current_position < ventilation_position:
                ...
                mem["last_command_time"] = now.isoformat()
                await self.hass.services.async_call("cover", "set_cover_position", {"entity_id": entity_id, "position": ventilation_position}, blocking=False, context=Context())
            elif is_opening_detected and current_position >= ventilation_position:
                # Bereits höher als Lüftungsposition - als abgeschlossen markieren, nichts tun
                mem["ventilation_initiated_today"] = now.date().isoformat()
                mem["ventilation_stopped_today"] = now.date().isoformat()
                _LOGGER.info("Ventilation skipped for %s: blind already at or above ventilation position (%s >= %s)", entity_id, current_position, ventilation_position)

        if is_morning_ventilation_time and mem.get("ventilation_stopped_today") != now.date().isoformat():
            is_crossing = False
            # Kreuzung nur bei Aufwärtsfahrt aus dem Bereich UNTERHALB der Lüftungsposition
            if cover_state == "opening" and current_position >= (ventilation_position - 3) and (last_known_pos is None or last_known_pos < ventilation_position):
                is_crossing = True
        ...
        if is_night:
            ...
            # Lüftung Morgens (gleiche Bedingung wie in Abschnitt 1 oben)
            is_morning_ventilation = (
                config.get("enable_ventilation", False)
                and now.time() <= self._parse_time(config.get("ventilation_until"), time(10, 0))
                and now < open_time_today  # Öffnungszeit noch nicht erreicht
        ...
        if has_active_override:
            ...
            elif is_transition and (action_type == "shading" or last_target_pos == int(config.get("shading_target_position", 0))):
            ...
            # Exception 3: Watchdog error correction (when offline/recovering etc.)
                ...
                # ha_recovering already computed at line 734 - reuse it
