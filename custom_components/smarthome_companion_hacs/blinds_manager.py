...
class BlindsManager:
    ...
    async def _evaluate_blind(self, entity_id, config, is_watchdog_check=False, is_state_change=False):
        ...
        # On static states, clear the automatic transit flag so any next movement is correctly recognized as manual
        ...
        # Safely parse manual override config
        ...
        try:
            manual_pause_duration = int(config.get("manual_pause_duration", 60))
        except (TypeError, ValueError):
            manual_pause_duration = 60
        
        # Detect manual action: Ignore movements that are registered as automatic transits
        is_transit = cover_state in ["opening", "closing"] and mem.get("automatic_transit", False)
        if not is_transit:
            if abs(mem.get("last_managed_position", current_position) - current_position) > 5:
                if enable_manual_pause:
                    mem["override_until"] = now + timedelta(minutes=manual_pause_duration)
                    _LOGGER.info("Manual override detected for %s. Override active until %s", entity_id, mem["override_until"])
                mem["last_managed_position"] = current_position
                # Clear automatic transit just in case
                mem["automatic_transit"] = False
        ...
        # Handle active manual overrides (checks if current override is still active)
        if mem.get("override_until"):
            if now < mem["override_until"]:
                return  # Skip automation, manually blocked
            else:
                    ...
                    if temp_sensor_id:
                        ...
                        if temp_state:
                            if temp_state.domain == "weather":
                                if "temperature" in temp_state.attributes:
                                    try:
                                        temp = float(temp_state.attributes["temperature"])
