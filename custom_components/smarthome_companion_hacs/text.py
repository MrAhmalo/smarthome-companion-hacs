...
class BlindTimeText(TextEntity):
    # Explicitly prevent HA from restoring our entity state after restart.
    # HA's state restore would call async_set_value() with old cached data,
    # which would overwrite settings saved to disk while HA was offline.
    ...
    async def async_added_to_hass(self):
        # Do NOT restore HA's cached state - we always read live from store.
        # HA's state restore would call async_set_value() with old cached data,
        # overwriting settings that were changed while HA was offline.
...
class HubTextSetting(TextEntity):
    # Explicitly prevent HA from restoring entity state after restart.
    _attr_should_poll = False
